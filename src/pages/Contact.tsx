...
import { Phone, Mail, MapPin, MessageCircle, Send, CheckCircle, ArrowRight, HeadphonesIcon } from 'lucide-react';

const CONTACTS = [
  { icon: Mail,           label: 'Email',         value: 'info@alpha-01.com',  href: 'mailto:info@alpha-01.com',   color: '#FFD166', bg: 'rgba(255,209,102,0.08)', border: 'rgba(255,209,102,0.20)' },
  { icon: MessageCircle,  label: 'WhatsApp Only', value: '+966 57 869 5494',   href: 'https://wa.me/966578695494', color: '#25D366', bg: 'rgba(37,211,102,0.08)',  border: 'rgba(37,211,102,0.20)'  },
  { icon: HeadphonesIcon, label: 'Customer Care', value: '+1 650 995 5661',    href: 'tel:+16509955661',           color: '#06D6A0', bg: 'rgba(6,214,160,0.08)',   border: 'rgba(6,214,160,0.20)'   },
  { icon: MapPin,         label: 'Location',      value: 'Riyadh, Saudi Arabia', href: '#',                       color: '#FF6B6B', bg: 'rgba(255,107,107,0.08)', border: 'rgba(255,107,107,0.20)' },
];
...
export default function Contact() {
  ...
  return (
    <div style={{ background: '#030307' }} className="text-white min-h-screen pt-24 pb-20">
      {/* Background */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full blur-[140px] opacity-20" style={{ background: 'rgba(255,209,102,0.15)' }} />
        <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full blur-[120px] opacity-15" style={{ background: 'rgba(6,214,160,0.12)' }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, x: -28 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.15 }} className="flex flex-col gap-4">
            {CONTACTS.map((c, i) => {
              const Icon = c.icon;
              const isLink = c.href !== '#';
              const Comp: any = isLink ? 'a' : 'div';
              return (
                <Comp key={i} href={isLink ? c.href : undefined}
                  target={c.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                  className="card p-5 flex items-center gap-4 group glass-shine transition-all">
                  ...
                  {isLink && <ArrowRight size={16} className="ml-auto text-gray-700 group-hover:text-amber-400 group-hover:translate-x-1 transition-all" />}
                </Comp>
              );
            })}
            ...
            <div className="card p-6" style={{ background: 'rgba(37,211,102,0.04)', borderColor: 'rgba(37,211,102,0.2)' }}>
              <p className="text-green-400 font-bold text-sm mb-2" style={{ fontFamily: 'Urbanist, sans-serif' }}>⚡ WhatsApp Only — Fastest Response</p>
              <p className="text-gray-500 text-sm mb-4" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                For the quickest response, WhatsApp us. Our team replies in minutes, day or night. <strong className="text-white">Note: This number is WhatsApp only.</strong>
              </p>
            ...
            {/* Customer Care highlight */}
            <div className="card p-6" style={{ background: 'rgba(6,214,160,0.04)', borderColor: 'rgba(6,214,160,0.2)' }}>
              <p className="text-teal-400 font-bold text-sm mb-2" style={{ fontFamily: 'Urbanist, sans-serif' }}>🎧 Customer Care Hotline</p>
              <p className="text-gray-500 text-sm mb-4" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                For billing, complaints, and escalations — call our dedicated customer care line.
              </p>
              <a href="tel:+16509955661"
                ...
                style={{ background: 'rgba(6,214,160,0.10)', border: '1.5px solid rgba(6,214,160,0.35)', color: '#06D6A0', fontFamily: 'Urbanist, sans-serif' }}>
                <HeadphonesIcon size={15} />+1 650 995 5661
              </a>
            </div>
          </motion.div>
