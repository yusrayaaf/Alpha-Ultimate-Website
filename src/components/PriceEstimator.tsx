...
const Select = ({ label, value, setFn, opts, color = '#06D6A0' }: any) => {
          ...
          <motion.ul initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            {opts.map((opt: string) => (
              <li key={opt} onClick={() => { setFn(opt); setOpen(false); }}
                className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-teal-400/8 transition-colors ${value === opt ? 'text-teal-400 font-semibold' : 'text-gray-400'}`}>
                {opt}
              </li>
            ))}
          </motion.ul>
